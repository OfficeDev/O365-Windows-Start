...
- 企業イントラネットと企業アカウントのサポートを追加 `AuthenticationContext` オブジェクトの `UseCorporateNetwork` プロパティを True に設定 プロジェクトにエンタープライズ認証、プライベート ネットワーク、共有ユーザー証明書機能の宣言を追加 詳細については、[アプリ機能の宣言 (Windows ランタイム アプリ)](http://aka.ms/vaha2s) をご覧ください。
